package zmaster587.advancedRocketry.mixin.core;
...
public class MixinConfigPlugin implements IMixinConfigPlugin {
    ...
    private static final String PACKAGE_PREFIX = "zmaster587.advancedRocketry.";
    ...
    private static final ImmutableMap<String, Consumer<PotentialMixin>> MIXIN_PROCESSING_MAP = ImmutableMap.<String, Consumer<PotentialMixin>>builder()
            ...
            .put("Lorg/dv/minecraft/logisticsbridge/annotation/LateMixin;", p -> p.isLate = true)
            .build();
    ...
    private void considerClass(String pathString) throws IOException {
        try (InputStream stream = MixinConfigPlugin.class.getClassLoader().getResourceAsStream("zmaster587/advancedRocketry/mixin/" + pathString)) {
            if (stream == null)
                return;
    ...
    private static void writeOrderedProperties(Properties props, OutputStream stream) throws IOException {
        try (PrintWriter writer = new PrintWriter(stream)) {
            writer.println("# " + Reference.NAME + " config file");
            writer.println();
    ...
    public void onLoad(String s) {
        if (allMixins.isEmpty()) {
            try {
                URI uri = Objects.requireNonNull(MixinConfigPlugin.class.getResource("/mixins." + Reference.MOD_ID + ".json")).toURI();
                FileSystem fs;
                try {
                ...
                List<Path> list;
                Path basePath = fs.getPath("zmaster587", "advancedRocketry", "mixin").toAbsolutePath();
            ...
            File targetConfig = new File(Launch.minecraftHome, "config" + File.separator + Reference.MOD_ID + ".properties");
            try {
    ...
    public List<String> getMixins() {
        ...
        if (phase == MixinEnvironment.Phase.DEFAULT) {
            ...
            List<String> list = allMixins.stream()
                    ...
                    .map(clz -> clz.replace("zmaster587.advancedRocketry.mixin.", ""))
